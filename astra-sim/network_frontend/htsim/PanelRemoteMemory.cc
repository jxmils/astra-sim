...
PanelRemoteMemory::PanelRemoteMemory(
...
void PanelRemoteMemory::set_sys(int id, Sys* sys) {
  sys_map[id] = sys;
}

void PanelRemoteMemory::issue(
  ...
  if (mem_type == NO_MEMORY_EXPANSION) {
    ...
    if (ongoing_transaction[nid]) {
      ...
      sys->register_event(this, EventType::General, new MemoryCallData(wlhd->sys_id), runtime);
...
void PanelRemoteMemory::call(EventType type, CallData* data) {
  MemoryCallData* md = static_cast<MemoryCallData*>(data);
  const int done_sys_id = md->sys_id;
  delete md;
  if (mem_type == PER_NODE_MEMORY_EXPANSION) {
    int nid = done_sys_id / num_npus_per_node;
    if (!pending_requests[nid].empty()) {
      ...
      sys->register_event(this, EventType::General, new MemoryCallData(pmr.wlhd->sys_id), runtime);
...
uint64_t PanelRemoteMemory::get_remote_mem_runtime(uint64_t tensor_size) {
